...
ari_narrate <- function(script, slides, 
  ...
  if (tools::file_ext(script) %in% c("Rmd", "rmd") & missing(slides)) {
  ...
  if (tools::file_ext(script) %in% c("Rmd", "rmd")) {
  ...
  #call special ari_spin fun
  ari_spin_descript(images = img_paths, paragraphs = paragraphs, 
